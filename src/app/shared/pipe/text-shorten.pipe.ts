...
export class TextShortenPipe implements PipeTransform {
  ...
  transform(input: string, lengthToShortenTo: number, ignoreCarriageReturn = true) {
    return this.getShortenedString(input, lengthToShortenTo, ignoreCarriageReturn);
  }

  public getShortenedString(input: string, lengthToShortenTo: number, ignoreCarriageReturn = true): string {
